...
class _NFCscannerState extends State<NFCscanner> {
  ...
  Future<bool> setUID(int uid) async {
    try {
      ...
      // Dein existierendes Modell weiterverwenden:
  ...
  Future<bool> deleteTag() async {
    ...
    int? id = prefs.getInt('uidInt') ?? 0;
    final ok = await delUID(id);
    if (!ok) return false; // Bei Fehler lokale Flags NICHT ändern
  ...
  Future<void> _onQrDetected(String raw) async {
    ...
    try {
      ...
      // // Falls dein QR JSON wie {"uuid":12345} liefert, ersetze obiges durch:
      // final map = jsonDecode(raw) as Map<String, dynamic>;
      // final uid = (map['uuid'] as num).toInt();
      ...
      Future<void> _finalizeConnection(int uid) async {
        ...
        if (!success) {
          setState(() => isPending = false);
          // iOS: Scanner wieder aktivieren, damit Nutzer neu scannen kann
          if (Platform.isIOS) {
            _qrLocked = false;
      ...
      await _qrController?.stop(); // Kamera pausieren
  ...
  // Simulate the scanning and connection process
  Future<void> scanTag(BuildContext context) async {
    setState(() {
      isPending = true; // Show the pending icon
    });
    Convert convert = Convert();
    NfcManager.instance.startSession(onDiscovered: (NfcTag tag) async {
      final prefs = await SharedPreferences.getInstance();
      NfcA? tech = NfcA.from(tag);
      final String identifier = tech!.identifier
          .map((e) => e.toRadixString(16).padLeft(2, '0'))
          .join('');
      int ud = convert.hexToDecimal(hexString: [identifier])[0];
      Provider.of<UserStore>(context, listen: false).changeTag(ud);
      final success = await setUID(ud);
      if (!success) return;
      await prefs.setInt('uidInt', ud);
      await prefs.setBool('uid', true);
      ...
      //await prefs.setString('initialRoute', '/mainpage');
      NfcManager.instance.stopSession();

      // Simulate a delay for the connection process
      await Future.delayed(const Duration(milliseconds: 500));
      setState(() {
        isPending = false; // Hide the pending icon // Connection successful
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    bool switcher = userStore.landing;
                              ...
                              child: isPending
                                  ...
                                  : !switcher
                                      ? (Platform.isIOS
                                          ...
                                          // ---------- Android: NFC wie bisher ----------
                                          : Column(
                                              key:
                                                  const ValueKey('android-nfc'),
                                              children: [
                                                SizedBox(
                                                  width: w * 0.7,
                                                  height: 8 + h * 0.05,
                                      ...
                                      : Center(
                                          // dein "Erfolgreich verbunden!"-Block bleibt
                                          child: Padding(
                                            padding:
                                                        ...
                                                        GestureDetector(
                                                          onTap: () async {
                                                            if (await deleteTag()) {
                                                              ...
                                                              // iOS: Scanner nach Trennen wieder aktivieren
                                                              if (Platform
                                                                  .isIOS) {
                                                                _qrLocked =
                                                                    false;
...
class _ThreeDotsLoaderState extends State<ThreeDotsLoader>
  ...
  Widget build(BuildContext context) {
    return Container(
      // Move the color into the BoxDecoration
      decoration: BoxDecoration(
